...
class HybridTaskScoringCascadeRoIHead(CascadeRCNN):
    def __init__(self,
                 num_stages,
                 backbone,
                 mask_iou_head,
                 ...
                 mask_info_flow=True,
                 **kwargs):
        super(HybridTaskScoringCascadeRoIHead, self).__init__(num_stages, backbone, **kwargs)
    ...
    def init_weights(self, pretrained):
        super(HybridTaskScoringCascadeRoIHead, self).init_weights(pretrained)
    ...
    def _mask_forward_train(self,
        ...
        if stage != 2 or self.mask_iou_head is None: 
            return loss_mask
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        if self.with_mask:
            if det_bboxes.shape[0] == 0:
                mask_classes = self.mask_head[-1].num_classes - 1  # NOTE
                ...
                merged_masks = merge_aug_masks(aug_masks,
                                               ...
                                               self.test_cfg.rcnn)
                merged_masks_wo_sigmoid = merge_aug_masks(aug_masks_wo_sigmoid, [img_metas]*self.num_stages, self.test_cfg.rcnn)
                ...
                mask_results = dict(mask_pred=merged_masks_torch, mask_feats=mask_feats)
            ...
            ms_maskscore_result['ensemble'] = mask_scores

        if self.with_mask:
            results = ms_bbox_result['ensemble'], (ms_segm_result['ensemble'], ms_maskscore_result['ensemble'])
        else:
